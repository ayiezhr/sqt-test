    ...
    <table border="1" id="adjustable">
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                echo "<tr>";
                echo "<td>" . $numrow . "</td><td>". $row["room_ID"]."</td>
                <td>" . $row["booking_Date"] . "</td>
                <td>" . substr($row["booking_Time_Start"], 0, 5) .   "</td>
                    <td>" . substr($row["booking_Time_End"], 0, 5) . "</td><td>" . $row["booking_Duration"] . "</td><td>" . $row["booking_Status"] . "</td>";
                echo '<td>';
